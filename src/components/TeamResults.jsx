...
//       // Automatically upload file after selection
...
//     formData.append("id", "1"); // Modify ID if needed

//     try {
//       const response = await fetch("http://183.83.216.29:7777/user/enroll?id=1", {
//         method: "POST",
//         body: formData,
//       });
...
//         <Button variant="dark" className="w-100 mb-2" as="div" disabled={isUploading}>
//           {isUploading ? "Uploading..." : uploadedFile ? uploadedFile.name : "Upload New Photo"}
//         </Button>
//       </label>
...
function TeamResults() {
  // Modified initial state to handle API response
  ...
  const uploadImage = async (file) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedResults = apiData.map((item, index) => ({
          id: index + 1,
          image: `http://183.83.216.29:7777/api/images/download/${item.imagePath}`,
          dateTime: formatDateTime(item.date),
          location: item.location || "Not specified",
          age: item.age,
          gender: item.gender === "MALE" ? "Male" : "Female",
          camera: item.camera || "Not specified",
          beard: "NIL",
          faceQuality: item.faceQuality,
          expanded: index === 0 // First result expanded
        }));
  ...
  // Keep your existing component code exactly as is from here...
  const ExpandedCard = ({ result }) => (
    // Your existing ExpandedCard JSX exactly as is
    <Card className="result-card">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2 className="mb-0">Matched - Results</h2>
        <span className="text-muted">{results.length} Results Found</span>
          ...
          <Col md={3}>
            <div className="image-container">
              <Image
                ...
                className="result-image-large rounded-circle"
              />
              <div className="eye-overlay"
                onClick={() => handleEyeClick(result)}
                style={{ cursor: "pointer" }}>
  ...
  const ModalContent = () => (
    modalData && (
      <Modal className='model-team-results' size='lg' show={showModal} onHide={() => setShowModal(false)} centered>
        <Modal.Header className='matched-ui-header-result'>
          <button
            type="button"
            className="btn-close custom-close-btn"
  ...
  return (
    <Container fluid className="p-4">
      ...
      <Offcanvas show={show} onHide={handleClose} placement="end" style={{ width: '60%' }}>
        <Offcanvas.Header className='matched-ui-header' closeButton>
          <Offcanvas.Title>Matched - Results</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
